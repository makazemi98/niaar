...
if (!function_exists('getPermissionsToInquiry')) {
    /**
     * @return array
     */
    function getPermissionsToInquiry()
    {
        $current_params = Route::current()->parameters();
        if (array_key_exists('inquiry', $current_params)) {
            $inquiry = $current_params['inquiry'];
            $user = Auth::user();

            $statusStage = array_search($inquiry->stage, InquiryStatusesEnum::manualUpdateStatusByUser());

            $prevStageIndex = $statusStage - 1 < 0 ? 0 : $statusStage - 1;
            $nextStageIndex = $statusStage + 1 >= count(InquiryStatusesEnum::manualUpdateStatusByUser()) ? count(InquiryStatusesEnum::manualUpdateStatusByUser()) - 1 : $statusStage + 1;

            $prevStage = InquiryStatusesEnum::manualUpdateStatusByUser()[$prevStageIndex];
            $nextStage = InquiryStatusesEnum::manualUpdateStatusByUser()[$nextStageIndex];

            $allPermissions = $user->getAllPermissions()->whereIn("name", ['update-status:' . $prevStage, 'update-status:' . $nextStage]);
